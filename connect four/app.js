  ...
  (function(index){
  	...
  	squares[i].onclick = function(){
  		...
  		if(squares[index + 7].classList.contains('taken')) {
  			...
  			// if the square below your cuurent square is not taken, you cant go here
...
// check the board for a win or lose function
function  checkBoard(){
	// make const that shows all wininig  arrays
	const winingArrays = [
	...
	// now take the 4  values in each winingArrays and plug them into the squers
	for(let y = 0; y < winingArrays.length; y++) {
		...
		if(square1.classList.contains('player-one') &&
			...
			// if they do , player-one is passed as the winnwe
        ...
        // now check to see if they all have the class name of player-two
        	...
        	//if they do player-two  is passed on as the inner
